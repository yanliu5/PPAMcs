...
#########################################################################
Simulation <- function(N,opt,alpha=0,v.seq,w.seq,u.seq){
  # N = 200; opt = 1
  ...
  # ===================================================================
	...
	tt0   <- Sys.time()
	res 	<- BIC.Cheng.Wang.SAT(lower,upper,alpha,di,Z,V,W,U,v.seq,w.seq,u.seq)
	tt1 	<- Sys.time()
...
if(cluster==1){
  source("/data/gpfs/home/yliu23/caseoneadd/Cheng_Yan_Add.txt")
	source("/data/gpfs/home/yliu23/caseoneadd/PH2_functions.txt")
	...
	source("Chris_CS_Support_PH.txt")
	source("PH2_functions.txt")
...
while(jj<=tot){
	...
	while(opt <= 3){
	  if(FALSE){
		...
		#res_200 <- Simulation(N=200,opt,alpha,v.seq,w.seq,u.seq)
		if(class(res_200)!="try-error"){
		  res_400 <- try(Simulation(N=400,opt,alpha,v.seq,w.seq,u.seq),silent=TRUE)
			#res_400 <- Simulation(N=400,opt,alpha,v.seq,w.seq,u.seq)
		}else{
		  res_400 <- res_200
		}
		...
		if(class(res_200)!="try-error"&class(res_400)!="try-error"){
			...
			if(cluster==1){
				write(res_200$save1,file=paste0("PH_N200_save1_mod",opt,".txt"),append=TRUE)
				write(res_200$save2,file=paste0("PH_N200_save2_mod",opt,".txt"),append=TRUE)
				write(res_200$save3,file=paste0("PH_N200_save3_mod",opt,".txt"),append=TRUE)
				
				write(res_400$save1,file=paste0("PH_N400_save1_mod",opt,".txt"),append=TRUE)
				write(res_400$save2,file=paste0("PH_N400_save2_mod",opt,".txt"),append=TRUE)
				write(res_400$save3,file=paste0("PH_N400_save3_mod",opt,".txt"),append=TRUE)
			}
			
			if(cluster==0){
			
				write(res_200$save1,file=paste0("local/PH_N200_save1_mod",opt,"_Set",set,".txt"),append=TRUE)
				write(res_200$save2,file=paste0("local/PH_N200_save2_mod",opt,"_Set",set,".txt"),append=TRUE)
				write(res_200$save3,file=paste0("local/PH_N200_save3_mod",opt,"_Set",set,".txt"),append=TRUE)
		...
		res_800 <- try(Simulation(N=800,opt,alpha,v.seq,w.seq,u.seq),silent=TRUE)
		if(class(res_800)!="try-error"){
		  write(res_800$save1,file=paste0("PH_N800_save1_mod",opt,".txt"),append=TRUE)
			write(res_800$save2,file=paste0("PH_N800_save2_mod",opt,".txt"),append=TRUE)
			write(res_800$save3,file=paste0("PH_N800_save3_mod",opt,".txt"),append=TRUE)
			print(paste0("Finish dataset ",jj," mod ",opt))
			opt <- opt + 1
	  }
	}
